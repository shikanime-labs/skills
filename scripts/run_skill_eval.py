...
def assert_one(text, s, baseline=None):
    ...
    if "description starts with 'use when'" in t:
        ...
        ok = d.startswith("Use when") and len(d) <= 200
        return ok, f"desc={len(d)}c use_when={d.startswith('Use when')}"
    ...
    if "heading" in t and "'##" in t:
        ...
        if " or " in t and t.count("'##") >= 2:
            wants = re.findall(r"'##\s*([^']+)'", t)
            ok = any(any(w.strip() == h.strip() for h in s["headings"]) for w in wants)
            return ok, f"any of {wants} present={ok}"
        want = re.search(r"'##\s*([^']+)", t)
        if want:
            ok = any(want.group(1).strip() == h.strip() for h in s["headings"])
