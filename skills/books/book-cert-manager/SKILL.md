...
- Install (static): `kubectl apply -f
  https://github.com/cert-manager/cert-manager/releases/download/v1.21.1/cert-manager.yaml`
- Install (Helm OCI): `helm install cert-manager
  oci://quay.io/jetstack/charts/cert-manager --version v1.21.1 --namespace
  cert-manager --create-namespace --set crds.enabled=true`
...
- `references/10-trust.md` — trusting private CAs, trust-manager, not mounting
  the server Secret.
- `references/11-trust-encoding-and-ca-bundles.md` — CA-bundle encoding
  failures: when cert-manager's ClusterIssuer can't reach Let's Encrypt
  because ISRG Root X1 is missing from the trust-manager-synched bundle;
  root cause, fix (add ISRG ConfigMap as Bundle source), and verification.
- `references/11-ca-rotation-reissue.md` — CA rotation reissue gap:
  cert-manager does not auto-reissue leaf certs when the CA rotates;
serial-comparison CronJob pattern and alternatives (cmctl renew, long CA
duration).
