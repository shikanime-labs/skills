...
- **OCI registry (source of truth):**
  `oci://quay.io/jetstack/charts/cert-manager:v1.21.1`. Published immediately on
  release.
- **Legacy HTTP repo:** `https://charts.jetstack.io` — updated a few hours after
  OCI; avoid for latest. Very old versions (< v1.12) only here.
...
`kubectl delete crd issuers.cert-manager.io clusterissuers.cert-manager.io
