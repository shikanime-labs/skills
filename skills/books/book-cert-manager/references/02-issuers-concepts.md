...
- `Issuer` — **namespaced**. Can only issue `Certificate`s in the same
  namespace. You need one per namespace that requests certs.
...
- **Built-in** (in `cert-manager.io` group): CA, SelfSigned, ACME, Vault,
  Venafi.
