...
1. Route **rule-level** (HTTPRoute/GRPCRoute/TCPRoute with `sectionName` on a
   rule)
2. Route-level (no `sectionName`)
3. Then by attachment path:

- Via ListenerSet: ListenerSet listener-level → ListenerSet-level →
  Gateway-level
...
- Merge types:
- **StrategicMerge** — Kubernetes strategic merge patch semantics (arrays merged
  intelligently)
...
`EnvoyExtensionPolicy`, `EnvoyGateway`, `EnvoyPatchPolicy`, `EnvoyProxy`,
`HTTPRouteFilter`, `SecurityPolicy`. All translate to xDS via the control plane.
