...
Distilled from README, docs/flags, charts values, and tutorials. Every flag has
an `EXTERNAL_DNS_*` env equivalent (e.g. `--dry-run` →
...
- **Registry**: use `txt` unless you have a reason (aws-sd/dynamodb for
  AWS-native ownership, `noop` only for fire-and-forget with no conflict risk).
- **Policy**: `upsert-only` (Helm default) NEVER deletes — use `sync` if you
  want garbage collection.
- **txt-owner-id**: set a stable, cluster-unique value. Changing it or
  `--txt-prefix` forfeits ownership of existing records.
- **CNAME/TXT clash on ELB/ALB**: `--txt-prefix` (CNAME can't co-exist with
  other records).
...
`provider.name`, `domainFilters`, `excludeDomains`, `sources`, `policy`,
`registry`, `txtOwnerId`, `txtPrefix`/`txtSuffix`, `annotationPrefix`,
`interval`, `env`, `extraArgs`, `rbac.create`, `serviceMonitor.enabled`,
`namespaced`, `sourceNamespace`, `managedRecordTypes`, `provider.webhook.*` (for
webhook providers). Default `policy` in chart is `upsert-only`.
