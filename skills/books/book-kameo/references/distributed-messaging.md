...
- The UUID string **must be unique within the crate** to avoid routing
  conflicts.

## Why `#[remote_message]` is needed

Kameo lets actors handle many message types without a central enum. At
deserialization time the exact type is unknown, so Kameo uses the **`linkme`**
crate to build, at link time, a HashMap:

```text
HashMap<RemoteMessageRegistrationID, RemoteMessagesFns>
```

- `RemoteMessageRegistrationID` = actor ID + message ID (from `RemoteActor` +
  `#[remote_message]`).
- `RemoteMessagesFns` = function pointers for the `ask`/`tell` handlers of that
  actor+message.
...
- Same `ask`/`tell` semantics as local (see `references/requests.md`); `tell`
  errors are still panics remotely.
- Distributed overview: `references/distributed-overview.md`; swarm:
  `references/distributed-swarm.md`; registry:
