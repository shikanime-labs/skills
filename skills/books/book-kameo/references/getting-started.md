...
- Distributed features need `features = ["remote"]`; observability needs
  `console`/`metrics`/`hotpath`.
...
- `#[actor(name = "MyAmazingActor")]` — custom name for logging (default: struct
  ident).
...
- `handle` takes `&mut self` (exclusive state access), the message, and a
  `Context`.
- README's first example uses `use kameo::actor::Spawn;` to bring `spawn` into
  scope; `prelude::*` covers the same.
