...
- `type Reply: Reply` — must implement the `Reply` trait (see
...
`handle` is async; you can `.await` I/O, other actors, etc. But **awaiting a
long future inside `handle` blocks the mailbox** (head-of-line). To keep
responsive, use `ctx.pipe`:
...
Same mechanism, applies result via closure with `&mut self` (no new message
type).
...
- Actor kept alive until the future resolves; if already stopped, the
  message/continuation is skipped.

## Notes

- Prefer `ask`/`tell` for normal sends; `pipe` only when you must start async
  work without blocking the mailbox.
