...
Every type returned as an actor response must implement `Reply`. Ensures the
value can be sent back through the messaging system.
...
- Most std types already implement `Reply`; report gaps via issue if a std type
  doesn't.

## Error-as-panic rule

- `Result::Err` replies are treated distinctly from non-error replies.
- When a message is sent via **`tell`**, any error from the handler is
  interpreted as a **panic** → may stop the actor (tune via `on_panic`).
- With **`ask`**, a handler error is returned as `SendError::HandlerError` to
...
- Reply errors are the bridge to supervision; pair with `references/requests.md`
  and `references/supervision.md`.
