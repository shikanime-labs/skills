...
Local build: `docker build -t local/llama.cpp:full-cuda --target full -f
.devops/cuda.Dockerfile .` Defaults: `CUDA_VERSION=12.8.1`, `CUDA_DOCKER_ARCH` =
all supported.
...
- SYCL: pass `--device /dev/dri/renderD128:/dev/dri/renderD128 --device
  /dev/dri/card0:/dev/dri/card0` and `--n-gpu-layers 99`. Intel GPU driver must
