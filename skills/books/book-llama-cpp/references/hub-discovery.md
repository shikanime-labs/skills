...
Example:
<https://huggingface.co/models?search=Qwen&apps=llama.cpp&num_parameters=min:0,max:24B&sort=trending>
...
Do this by reading the URL itself, not by assuming the UI rendered in a browser.
...
On the local-app page, the hardware compatibility section can expose entries
such as:
...
- If the HF hardware compatibility panel is missing because the user has no
  ...
  it, still use the tree API plus generic quant guidance from `quantization.md`.
- If the repo already has GGUFs, do not jump straight to conversion workflows.
- `llama-cli -hf` flattens `repo:quant` into one cache/filename scheme;
  collisions and the `HF_HOME`-must-be-exported trap: see
  `references/llama-cli-flatten.md`.
