...
Use the file-specific form when the repo has custom naming or when you already
extracted the exact filename from the tree API.
...
Every `llama-server` flag also has a `LLAMA_ARG_*` env equivalent — useful to
run the server with `command: [llama-server]` and no `args:` in containers.
Mapping table and the `--no-models-autoload` gateway trap:
`references/llama-server-env.md`.
