...
`--spec-draft-conf-min P` truncates a block at first position below confidence
P.
...
`--spec-ngram-simple-size-m` (draft, def 48), `--spec-ngram-simple-min-hits`
(def 1).
- `ngram-map-k`: `--spec-ngram-map-k-size-n` (12), `-size-m` (48), `-min-hits`
  (1).
