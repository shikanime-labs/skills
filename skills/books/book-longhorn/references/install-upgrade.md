...
Rancher Catalog, kubectl, **Helm**, Helm Controller, Fleet, **Flux**, ArgoCD,
airgap. All Longhorn CRs/pods live in namespace `longhorn-system`.
...
`/dev`, `/proc` (ro), `/var/lib/longhorn`, `/boot` (ro), `/etc` (ro),
`/lib/modules`, `/sys`.
...
- Kernel ≥ **5.19** for NVMe/TCP; ≥ **6.7** for stability (avoids SPDK #3116
  memory issue).
- Huge pages: **1024 × 2 MiB** pages (≈2 GiB) reserved per node for SPDK.
- IOMMU group isolation: NVMe must not share an IOMMU group with a PCIe bridge
  (else `failed to bind NVMe disk` / error -22 → use AIO driver).
...
Enable V2: set `defaultSettings.v2DataEngine: true` in Helm values (or the
`longhorn-system` setting after install).

## Upgrade

- **Supported path: only v1.11.x → v1.12.1.** Other versions must step to
  v1.11.x first.
- V1 engine live-upgrade supported; **V2 engine live-upgrade NOT supported —
  detach first.**
- Pre-upgrade job (Helm/Rancher) blocks unsupported paths; logs reason, can
  rollback.
- Set `spec.failurePolicy: abort` (NOT `reinstall`) on HelmChart/HelmRelease
  upgrades.
...
- **Always back up volumes before upgrading.** Do not modify the default
  StorageClass parameters (provisioner change is Forbidden; delete deprecated SC
  first).
