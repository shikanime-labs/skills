...
- "Why does `environment.systemPackages` merge but `services.httpd.adminAddr`
  error?" → nixos-configuration.
- "How do `override`, `overrideAttrs`, and overlays differ?" →
...
- Package-type options (like `services.gpg-agent.pinentryPackage`) require a
  package whose `meta.mainProgram` is executable compatible with the role.
  Verify first: `nix eval nixpkgs#<pkg>.meta.mainProgram`. Do not assign
  general-purpose CLIs to specialized-package options just because they happen
