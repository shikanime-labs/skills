...
*Distilled from NixOS Manual: configuration syntax, module abstraction,
modularity.*

## The configuration file

- `/etc/nixos/configuration.nix` is a **Nix expression**: a function returning
  an attrset of option definitions.
...
- **Integer**: `boot.kernel.sysctl."net.ipv4.tcp_keepalive_time" = 60;` — note
  the quotes: the key is a *literal kernel name*, not a nested option.
- **Set**: `fileSystems."/boot" = { device = "/dev/sda1"; fsType = "ext4";
  options = ["rw" "relatime"]; };`
- **List**: whitespace-separated — `boot.kernelModules = [ "fuse" "kvm-intel"
  ];`.
- **Package**: via `pkgs` — `environment.systemPackages = [ pkgs.thunderbird
  pkgs.emacs ];` or swap a service's package: `services.postgresql.package =
  pkgs.postgresql_14;`.
...
- A `let` may appear wherever an expression is allowed; NOT as an attribute
  name.

## Modularity (configuration.nix IS a module)

- Split large configs: `imports = [ ./vpn.nix ./kde.nix ];`. Submodules share
  syntax.
- **List-typed options MERGE** across modules (concatenated; the importing
  module's value is appended *last*). Use `mkBefore` to prepend:
