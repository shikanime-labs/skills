...
- `!!`, `!$`, `!<n>`, `!<-n>`, `!<str>` history substitution (inserted, not
  auto-run — review first).
- `Ctrl/⌘+R` reverse search; `Ctrl/⌘+O` edit line in `$env.EDITOR` (inserted,
  not auto-run).
- Windows Git Bash externals (`ln`,`grep`,`vi`): add `$env.Path = ($env.Path |
