...
Source: `book/navigating_structured_data.html`. Cell-paths are Nu's query
language.
...
- Table: **a list of records**. So anything valid on a list is valid on a table;
  not vice-versa.
...
- `get <cell-path>`: returns the **value** at the path. `get 1` on a table =>
  record (same as `$data.1`).
- `select <cell-paths>`: returns the **same structure** (table/list/record)
  possibly smaller. `select 1` on a table => a new one-row table with fresh
  0-based indices.
- `select` accepts multiple cols/rows: `$data | select date condition 0 1`.
- `select` result indices are NOT original; use `enumerate` then `select` to
  preserve original index.

## Missing data

- `?` optional operator: `$data | get a?` => `null` if missing instead of error.
  Also in cell-path literals: `$.temps?.1`.
