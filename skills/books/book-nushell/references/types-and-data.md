...
Source: `book/types_of_data.html`. Nushell commands pass typed values (not just
text) through pipelines.
...
- **Tables are internally lists of records.** `get 0` on a table yields a
  `record`; on a list yields the first element.
...
- Cell-path: dot-separated row(int)/col(string) ids; leading `$.` disambiguates
  assignment to a var. `let cp = $.2` then `[foo bar goo] | get $cp`.
- Closure literal `{|args| expr}`; used by filters (`where`, `each`) and closes
  over outer scope.
- **Optional `?`** on a cell-path returns `null` if missing: `{a:5} | get a?` =>
  `5`; `get c?` => `nothing`. `== null` true.

## Type mismatch behavior

Operators are not universal; Nu errors on type mismatch (`can't convert
list<string> to string`). Check a command's accepted types with `help <cmd>` ->
Input/output types.
