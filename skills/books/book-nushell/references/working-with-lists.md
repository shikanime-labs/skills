...
Source: `book/working_with_lists.html`. Lists == single (unnamed) table column;
commands on columns also work on lists.

## Create

`[foo bar baz]` or `[foo, bar, baz]` (JSON-array compatible). Items split by
spaces/commas/linebreaks.
