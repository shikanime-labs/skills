...
- Double-quote escapes: `\" \' \\ \/ \b \f \r \n \t \u{X...}` (1-6 hex). `\0`
  via `\u{0}` or `(char nul)`.
- Bare/backtick words in command position run as external (or path).
  `true`/`trueX` are special tokens, not strings. Prefer quotes when
  programming.
- `^` sigil forces external: `^'C:\Program Files\exiftool.exe'` or `^$foo`. Also
  `run-external`.
...
- v0.61+: escape parens `\(` `\)` to print literal `()`: `$\"2+2 is (2+2)
  \(guessed!)\"`.
- `const x = $"(2kb)"` evaluates at parse time with default config (file
  sizes/datetimes use default units).
...
- `parse '{shell} {version}'` and `parse --regex '(?P<subject>\w+...) is
  (?P<adj>\w+)'`
