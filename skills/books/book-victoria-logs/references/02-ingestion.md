...
- Elasticsearch bulk: `POST /insert/elasticsearch/_bulk` (ES/OpenSearch bulk
  JSON). Stops on first error. `_time:"0"` ⇒ server-side current timestamp.
...
- `_msg_field` — field holding the message (usually `message` for
  Filebeat/Logstash). Comma list; first non-empty used. Falls back to `_msg`,
  then `-defaultMsgValue`.
- `_time_field` — field holding timestamp (usually `@timestamp`). Comma list.
  Falls back to `_time`, then ingestion time.
- `_stream_fields` — comma list of fields uniquely identifying the stream. If
  unset, taken from `_stream` field labels; if that's absent ⇒ default `{}`
  stream.
...
- `debug=1` — don't store; log ingested rows instead
