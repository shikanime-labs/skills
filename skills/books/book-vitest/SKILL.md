...
- `vitest` — watch mode in dev; `vitest run` — single run; `vitest related` /
  `vitest bench`.
- `vitest run --coverage` / `--shard=1/3` / `--changed HEAD~1` / `-t <pattern>`.
- A dedicated `vitest.config.*` FULLY OVERRIDES `vite.config.*` unless you
  `mergeConfig`.
...
- `references/vitest-overview-config.md` — getting started, config precedence
  (vite vs vitest config), requires/coverage.
- `references/vitest-cli.md` — commands (`run`/`related`/`bench`/`list`), key
  CLI options.
