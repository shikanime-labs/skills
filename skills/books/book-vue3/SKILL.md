...
Structured notes distilled from the official Vue 3 documentation
(vuejs.org/guide). Covers the core mental models and patterns an engineer needs
to author and review Vue 3 applications: reactivity, template syntax,
components, the Composition API, built-in components, scaling-up patterns, and
best practices.
...
docs for exhaustive signatures. Source is Vue 3 (Vue 2 reached EOL on
2023-12-31).
...
- Reactive primitive: `ref(0)` → access via `.value`; auto-unwrapped in
  templates.
...
- `ref()` needs `.value` in JS but NOT in templates (except nested
  non-top-level).
...
Load on demand with `skill_view(name="book-vue3",
...
special attributes, global/composition/options entry points. Load for
signatures.
