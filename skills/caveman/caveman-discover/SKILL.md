...
description:
  "Use when finding and labeling every LLM workflow in a repository so Caveman Cloud groups spend by workflow: inventory, name, propose, wire, verify."
version: 0.1.0
author: Hermes Agent
license: Apache-2.0
platforms:
...
|---|---|---|---|
| support-reply | answers tickets | src/bot/reply.ts:41 | defaultHeaders |
| nightly-digest | 02:00 summary | jobs/digest.ts:12 | defaultHeaders |
| eval-suite (review) | inferred from name | scripts/eval.ts:8 | env override |
```
...
| workflow | job | where |
|---|---|---|
| support-reply | answers tickets | src/bot/reply.ts:41 |
| nightly-digest | 02:00 summary | jobs/digest.ts:12 |
