...
name: caveman-setup
description:
  "Use when wiring a repository through the Caveman Cloud gateway so every LLM request is measured with no behavior change (/caveman-setup)."
version: 0.1.0
author: Hermes Agent
license: Apache-2.0
platforms:
...
- `GATEWAY` — the gateway base URL (e.g. `https://gateway.caveman.so` or
  `http://127.0.0.1:8787` )
- `CAVE_API_KEY` — the gateway auth secret (treat like any API key: env var
  only, never committed, never printed in full)
- `PROVIDER_KEYS` — `stored` (provider keys live encrypted in Caveman Cloud) or
  `byok` (this app sends its own provider key per request)
...
- Raw HTTP to `api.openai.com` , `api.anthropic.com` ,
...
const client = new OpenAI({
  ...
  apiKey: proces...KEY,  // stored: Cave key · byok: keep provider key
  defaultHeaders: {
...
client = anthropic.Anthropic(
    ...
    api_key=os.environ["ANTHROPIC_API_KEY"],  # stored: Cave key
    default_headers={
...
**Vercel AI SDK** — `createOpenAICompatible({ baseURL:`
${gw}/w/<app>/openai/v1`,
...
Verified: HTTP 200 · model <model> · <in> in / <out> out tokens
  (one real request)
...
the public catalog. <DASHBOARD>/getting-started flips to
"First request received."

Want spend split by workflow (e.g. support-reply vs nightly-digest), not just
by app? Say "discover workflows" — I'll fetch
<docs origin>/docs/discover-workflows.md and label every callsite by the job
it does.
```
