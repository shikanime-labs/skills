...
- **Force-updating an upstream `fix/*` branch you own**: `jj git push` has
  NEITHER `--force` NOR `--allow-new` in this jj version — both error. To
  replace an earlier draft on a branch you own, capture the current remote SHA
  via `git ls-remote --heads upstream <branch>`, then
  `git push --force-with-lease=refs/heads/<branch>:<EXPECTED_SHA> upstream <NEW_SHA>:refs/heads/<branch>`
  (the lease fails safe if the remote moved since you fetched). Re-sync with
  `jj git import`. jj's `@upstream` tracking view may show a stale/empty tip
  even after a real push — trust `git ls-remote`, not jj's view. Generic
  fallback + staleness notes live in `jj-common-workflows`.
