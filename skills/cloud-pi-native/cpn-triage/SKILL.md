...
item** and **determinable from the item's own content**. French conventions
(see `cpn-issue`). Never invent a value the repo does not have.
...
- **labels** — infer from title conventional prefix: `fix:`/`[BUG]`→`bug`;
  `feat:`/`[REQUEST]`→`enhancement`; `docs:`→`documentation`; `refactor:`→`refactor`;
