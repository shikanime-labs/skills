...
6. **Verify the link is real.** PRs in this repo are frequently linked to an
   issue that describes a _different_ change. If the linked issue doesn't
...
- The change replaced the legacy fake email `${slug}@${slug}` with
  ...
  login.
- PR #2403 ("use project owner email") → linked issue #2400, which is a _Vault
  GITLAB secret-path bug_ — **unrelated and mis-associated**. No prose rationale
...
- Fix direction: revert the robot email to a unique per-project fake
  (`${slug}@${slug}`), matching the legacy plugin
  `plugins/sonarqube/src/user.ts`.
