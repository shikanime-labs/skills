...
**Status update (2026-09-02, PR #2088 merged → `d845a1143`):** all 33 dangling
files from the findings below were DELETED, and the authelia hostname-replace
violation was flipped to `add /spec/hostnames/-` (zero hostname-replace ops
remain fleet-wide). Do not re-report those findings — rerun the scan for NEW
ones; the method below stays valid.
...
1. **27 orphaned tailnet `patch-netpol.yaml`** — SMP template adding
   ...
   Live-cluster note: envoy data-plane pods (label `app.kubernetes.io/name=envoy`,
   ns `envoy-gateway-system`) reached app pods even though the app netpol had
   no matching ingress entry — verified with a curl probe pod. Netpol
   enforcement is partial; treat these as latent, not active, breakage.
