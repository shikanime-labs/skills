...
8. **Updating an existing PR — stay inside ghstack's tracking.**
   `ghstack submit` identifies which PR to update via commit metadata (the
   `ghstack` trailer in the commit message). If you rebase or amend the commit
   outside ghstack's flow (e.g., `git rebase origin/main` then
   `git commit --amend`), the metadata link can break and `ghstack submit` will
   create a **new** PR instead of updating the existing one.

   **Correct update flow:**

   ```bash
   ghstack checkout <PR_NUMBER>     # checks out the tracked commit
   git checkout -b pr-fix           # create a local branch
   git rebase origin/main           # rebase onto latest main
   # ... apply fixes ...
   git add -A
   git commit --amend --no-edit     # amend the tracked commit (preserves metadata)
   ghstack submit --force           # updates the existing PR
   ```

   **If ghstack creates a new PR anyway** (metadata was lost): close the new PR,
   delete its remote branches, and use `ghstack checkout <ORIGINAL_PR>` to start
   from the tracked commit again. Do NOT try to force-push to `gh/user/N/head`
   manually — this corrupts ghstack's branch tracking.

   **If the original PR is unrecoverable** (e.g., commits show
   `[ghstack-poisoned]`): close it, and use `ghstack submit` from the
   checked-out tracked commit to create a fresh PR. Then reference the old PR
   number in the new PR's description.

9. **Squash formatting fixes into the content commit — never a separate PR.**
   When CI fails due to formatting (e.g., markdownlint, treefmt), apply fixes
   and `git commit --amend --no-edit` into the existing tracked commit. Then
   `ghstack submit --force`. Do NOT create a second commit for formatting fixes
   — the user wants 1 commit == 1 PR, not a formatting-fix PR stacked on top.

10. **Splitting a stack into separate PRs.** When asked to split changes into
