...
- State the contract: what should happen, what does, and the smallest delta
  between them.
- Reproduce consistently with the exact command (`pytest -q tests/test_x.py::t`,
...
- **Boundary logging** in multi-component systems (API → service → DB): add one
  log line per boundary to learn _where_ it breaks, then investigate only that
...
- **Do not fan out parallel hypotheses.** The defect is singular; dispatching
  ...
  produces contradictory evidence. Fan-out is only correct for _attribution_ —
...
- **TypeScript:** reproduce in the playground with only the types + the failing
  expression. Surface the type with `satisfies` / `as const`; let `strict` tell
