...
(they already encode the shikanime trust-boundary lessons: Keycloak/JWKS timeouts,
client-binding on `aud`/`azp`, Prisma vs contract types).
...
- Keycloak/JWT/OIDC: token bound to backend `aud`/`azp`/client-id, not
  realm-only; JWKS fetch has a timeout + safe cache/refresh (no hang-on-network).
  Both cache-hit and cache-miss `kid` paths verified.
...
List every match as a `blocking` finding anchored at file:line. If clean,
state "security scan: clean" in the summary.
