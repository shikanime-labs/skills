...
Orchestrator over `sks-issue`/`sks-pr`/`sks-pr-triage`: issue → PR → triage. Environment facts (org identity, repo paths, toolchain, branch protection, push policy) live in `sks-constitution`.
