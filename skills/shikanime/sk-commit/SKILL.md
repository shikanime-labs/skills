...
- `gh` authenticated; the canonical remote is `shikanime-labs` or
  `shikanime-studio`
  (NOT a personal fork). Note: local path may say `shikanime-labs` while the gh
  remote is `shikanime-studio` (e.g. nix-containers) — trust the gh remote.
- jj repos: ensure the branch is tracked on the upstream remote
  (`jj bookmark track <branch> --remote=upstream`) before any push.
...
- **Doc repos** (e.g. `documentation-interne-socle`,
  ...
  `doc:` prefix, otherwise same shape.
...
- `manifests` repo: gitlint enforces a **body** (rule B6 "body message is
  ...
  plain title, no conventional prefix — this overrules the no-body default.
  When you do include `Related:`, use full issue URLs — never bare `#N` or
  `owner/repo#N` shorthand (both render broken on GitHub):
  `Related: https://github.com/<org>/<repo>/issues/N` (same repo) or
`Related: https://github.com/owner/repo/issues/N` (cross-repo). Multiple links:
  comma-separate on one line if it fits 80 cols, else one `Related:` per URL
  (split to satisfy the 80-col wrap).
- **`gh stack` is the landing path** for the commit (see `sk-pr` /
  `sk-dev-workflow`): it
reads the commit subject/body to seed PR title/description, enforcing PR↔commit
  parity. Author the commit to carry the full rationale.
- Stacked PRs are a **GitHub public-preview** feature (per docs.github.com). The
  `github/gh-stack` extension is released and installable (`gh extension install
  github/gh-stack`), but the feature is subject to change — fine for internal
  shikanime use, not a hard external contract yet.
...
**source of truth** — the PR title/body are derived from it later (see `sk-pr`),
   so state what/why cleanly here; do not leave details for the PR only.
   jj form — two `-m` blocks = subject paragraph + trailer paragraph:
   ```bash
   jj describe -m "<subject>" -m "Co-authored-by: Automata <automata@shikanime.studio>"
   ```
git form: `git commit -m "<subject>" -m "Co-authored-by: Automata
     <automata@shikanime.studio>"`.
3. Confirm the hook accepted it (`git log -1` / `jj log -1`).

## Push / landing
- **Do NOT push to `main`** unless the user explicitly authorizes
  ("push to main" / "land it") — then push directly, no PR needed.
...
- Pushing to a personal fork — shikanime policy is upstream-only.
- Forgetting `jj bookmark track` on jj repos -> push to upstream fails.
