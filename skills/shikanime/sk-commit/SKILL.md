...
- `gh` authenticated. Branches push to the fork remote; the org remote
  (`shikanime-labs` / `shikanime-studio`) receives `main` only. Local path
  may say `shikanime-labs` while the gh remote is `shikanime-studio`
...
- **Do NOT push to `main`** unless the user explicitly authorizes
  ("push to main" / "land it") — then push directly to the org remote, no PR
  needed.
...
- Forgetting `jj bookmark track <branch> --remote=fork` on jj repos -> push
  to the fork fails.
