...
- One clean change with a known shape → `sks-delegate` alone.
...
1. **Decide the sandbox shape** before opening anything:
   ...
   - Single trial, one hypothesis → one `sks-delegate` workspace.
...
2. **Open the sandbox** with the chosen primitive — never the main checkout:
   ...
   - One unit → `sks-delegate` (fresh `jj workspace add -r main@origin`).
...
- `sks-delegate` — the single-workspace primitive for one trial.
