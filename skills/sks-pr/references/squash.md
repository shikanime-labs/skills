# Squash, author/sign, verify, and pitfalls (PR finalize detail)

Load from `SKILL.md` before squashing a finalized PR branch, or when
...
jj squash -r <second> --message "<subject>"
```
...
jj describe -m "<subject>"   # set message, no editor
...
- **Squash to one commit**:
  `git reset --soft <base> && git add -A && git commit -m "<subject>"`.
...
- Assuming one repo's rules apply everywhere → false enforcement (e.g. forcing
...
- **Verify the linked issue actually matches the PR's code change.** A PR's
  linkage can point at an unrelated issue. Search the code (`jj file annotate`,
  `jj show <commit>`) and the commit for the actual intent, and link the
  _correct_ issue (or create one).
- **Doc repos may REQUIRE a `doc:` commit prefix** while the PR _title_ is
  conventional — probe the repo, don't assume one shape covers both.
...
- **Duplicate issues**: when a PR links a newer issue that duplicates an older
  ...
  `gh issue close <dup> --reason duplicate --comment "duplicate of #<canon>"`.
